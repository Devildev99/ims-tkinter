...
def update(id,v2,v3,v4,v5):
    ...
    try:
        cur=conn.cursor()
        q1="update `student` set `name`='"+nm+"',`roll`='"+roll+"',`email`='"+em+"',`phone`='"+ph+"' where `id`='"+id+"'"
        res=cur.execute(q1)
        if(res):
            conn.commit()
            conn.close()
...
def disp(s1,id):
    ...
    tk.Button( tk.Button(text="Update",fg="white",bg="orange",bd=2,font=("Arial",12),command=lambda:update(id,v2,v3,v4,v5)).place(x=150,y=250,width=100,height=25))
